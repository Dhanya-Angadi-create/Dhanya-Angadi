# 👋 Hi, I'm Dhanya Angadi

## 📚 About Me
I am a passionate **B.E. Electronics and Communication** student at **KLE Technological University**, actively exploring the fields of **Python**, **Artificial Intelligence**, and **IoT**.

## 🎓 Education
- **Degree:** B.E. in Electronics and Communication
- **University:** KLE Technological University
- **Focus Areas:** Python, AI

## 💻 Skills
- **Programming Languages:** Python and C programming and DSA
- **AI & Machine Learning:** Learning and developing skills

## 🚀 Projects

### 1. Fingerprint Door Lock System
- **Description:** A Python-based fingerprint authentication system for door lock control
- **Technologies:** Python, Fingerprint Sensor
- **Year:** 1st Year B.E.
- **Highlights:** Biometric authentication, Access control

### 2. Transformer Car
- **Description:** An autonomous or remote-controlled car with smart features
- **Technologies:** ESP32, Arduino IDE, IoT
- **Year:** 1st Year B.E.
- **Highlights:** Microcontroller programming, Real-time control

## 🔗 Connect With Me
- **LinkedIn:** [Dhanya Angadi](https://linkedin.com/in/Dhanya-Angadi)
- **LeetCode:** [Dhanya Angadi](https://leetcode.com/Dhanya-Angadi)
- **GitHub:** [@Dhanya-Angadi-create](https://github.com/Dhanya-Angadi-create)

## 🌱 Currently Learning
- Advanced Python programming
- Artificial Intelligence Tools

## 💡 Interests
- AI agents
---

**Feel free to explore my repositories and connect with me!** 🚀
